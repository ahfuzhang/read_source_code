...
type Block struct { // 看起来是完全 decode 后在内存中的数据
	bh blockHeader
	...
	headerData []byte  // ??? 这个是 block header 序列化后的信息吗?

	// Marshaled representation of timestamps.
	timestampsData []byte // 这个数据来自 timestamps.bin

	// Marshaled representation of values.
	valuesData []byte // 这个数据来自 values.bin
}
...
func (b *Block) Init(tsid *TSID, timestamps, values []int64, scale int16, precisionBits uint8) { // 从很多已经汇聚好的数据里创建 Block 对象
	b.Reset()
	b.bh.TSID = *tsid
	b.bh.Scale = scale
...
func (b *Block) deduplicateSamplesDuringMerge() { // 给定一个时间范围，在这个时间范围内，只取最大的 timestamp
	if !isDedupEnabled() {
		// Deduplication is disabled
		return
	}
	// Unmarshal block if it isn't unmarshaled yet in order to apply the de-duplication to unmarshaled samples.
	if err := b.UnmarshalData(); err != nil { // 重复调用 UnmarshalData 是没关系的
	...
	dedupInterval := GetDedupInterval() // 对于数据合并，这个功能可以设置为 30s  // !!! 非常好，降采样功能可以在这里完成
	if dedupInterval <= 0 {
		// Deduplication is disabled.
		return
	}
	srcValues := b.values[b.nextIdx:]
	timestamps, values := deduplicateSamplesDuringMerge(srcTimestamps, srcValues, dedupInterval) // 对两个数组去重
...
func (b *Block) UnmarshalData() error { // ??? 什么时候调用反序列化
