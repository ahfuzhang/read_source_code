...
func NewVMInsertServer(addr string, storage *storage.Storage) (*VMInsertServer, error) {  // vm-insert 的服务端
...
func (s *VMInsertServer) run() {  // 启动 insert 服务
	...
	for {
		...
		go func() {  // 每个连接一个携程
			...
			err = stream.Parse(bc, func(rows []storage.MetricRow) error {
				...
				return s.storage.AddRows(rows, uint8(*precisionBits))  // 解析数据，并写入 storage 对象
