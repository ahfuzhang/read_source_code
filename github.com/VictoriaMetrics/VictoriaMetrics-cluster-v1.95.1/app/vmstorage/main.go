...
func main() {
	...
	storage.SetDedupInterval(*minScrapeInterval)  // 当 minScrapeInterval 大于 0 的时候，会对数据去重
	storage.SetLogNewSeries(*logNewSeries)
	storage.SetFinalMergeDelay(*finalMergeDelay)  // 这个参数看起来有用
	...
	startTime := time.Now()
	strg := storage.MustOpenStorage(*storageDataPath, retentionPeriod.Duration(), *maxHourlySeries, *maxDailySeries)  // 打开数据文件的过程
	...
	vminsertSrv, err := servers.NewVMInsertServer(*vminsertAddr, strg)  // 写入用的 server
	if err != nil {
...
func requestHandler(w http.ResponseWriter, r *http.Request, strg *storage.Storage) bool {  // tsdb 内部的  url 访问入口
	path := r.URL.Path
	if path == "/internal/force_merge" {
		...
		go func() {
			...
			startTime := time.Now()
			if err := strg.ForceMergePartitions(partitionNamePrefix); err != nil {  // 仅 merge data 部分
