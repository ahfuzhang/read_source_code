...
// write amplification.
const finalPartsToMerge = 2 //??? 每次最多 merge 两块吗?
...
const maxPartSize = 400e9  // 372.5GB
...
type Table struct {
	...
	fileParts []*partWrapper // 包含多个目录的 parts 数据

	// This channel is used for signaling the background mergers that there are parts,
	// which may need to be merged.
	needMergeCh chan struct{} // 这里面有信号的时候，唤醒 merge 协程

	stopCh chan struct{}

	wg sync.WaitGroup  // 这个对象会等待 merge 协程结束
...
func (riss *rawItemsShards) addItems(tb *Table, items [][]byte) { // 加到内存的 table 中
	shards := riss.shards
...
func (ris *rawItemsShard) addItems(tb *Table, items [][]byte) [][]byte { // 在内存表中插入索引数据
	...
	for i, item := range items {
		...
		if len(ibs) >= maxBlocksPerShard { // 超过 256 个内存 block ，就开始写文件
			ibsToFlush = ibs
		...
		ib = getInmemoryBlock()
		if ib.Add(item) { // 在内存 block 中插入一条数据
			ibs = append(ibs, ib)
			continue
		}
		putInmemoryBlock(ib)
	...
	tb.flushBlocksToParts(ibsToFlush, false) // 把 256 个内存 block 进行 flash

	if len(ibsToFlush) > 0 {
		// Run assisted merges if needed.
		flushConcurrencyCh <- struct{}{}
		tb.assistedMergeForInmemoryParts()
		tb.assistedMergeForFileParts()  // part 文件 merge
		<-flushConcurrencyCh
	}
...
type partWrapper struct {
	...
	mustBeDeleted uint32  // 当设定这个标志后，没有引用计数时就会被删除
...
//
// The table is created if it doesn't exist yet.  // 打开  cur 目录
func MustOpenTable(path string, flushCallback func(), prepareBlock PrepareBlockCallback, isReadOnly *uint32) *Table { // index db 下的文件
	...
	// Open table parts.
	pws := mustOpenParts(path) // 打开  parts 文件夹  /data/sharding-00-restore/indexdb/179D7BFA38672AE6/(多个 parts 目录)

	tb := &Table{
		path:          path,
		flushCallback: flushCallback,
		prepareBlock:  prepareBlock,
		isReadOnly:    isReadOnly, // 指示是否是只读的
		fileParts:     pws,
		mergeIdx:      uint64(time.Now().UnixNano()),
		needMergeCh:   make(chan struct{}, 1), // 这里面有信号的时候，唤醒 merge 协程
	...
	tb.notifyBackgroundMergers() // 唤醒  index merge 协程
...
func (tb *Table) startBackgroundWorkers() {
	tb.startMergeWorkers()  // 启动 merge 协程
...
func (tb *Table) MustClose() {
	...
	tb.wg.Wait()  // 最终 close 的时候，在这里等待 merge 协程结束
...
func (tb *Table) AddItems(items [][]byte) { // 添加索引数据
...
func (tb *Table) getParts(dst []*partWrapper) []*partWrapper { // 引用计数，得到  parts 数组
	tb.partsLock.Lock()
	for _, pw := range tb.inmemoryParts {
		pw.incRef()
	}
...
func (tb *Table) mergePartsOptimal(pws []*partWrapper) error {
	...
	for len(pws) > 0 {
		...
		pwsChunk := pws[:n]
		pws = pws[n:]
		err := tb.mergeParts(pwsChunk, nil, true)  // table 上的 merge 方法
		if err == nil {
			continue
		}
...
func (tb *Table) flushBlocksToParts(ibs []*inmemoryBlock, isFinal bool) { // 内存中的  block 太多的时候
	if len(ibs) == 0 {
		return
	}
	...
	wg := getWaitGroup()
	for len(ibs) > 0 {
		n := defaultPartsToMerge // 一次 merge 15 个 内存 block
		if n > len(ibs) {
			n = len(ibs)
		}
		wg.Add(1)
		flushConcurrencyCh <- struct{}{}
		go func(ibsChunk []*inmemoryBlock) { // 这里最多传入 15 个 内存 block
			defer func() {
				<-flushConcurrencyCh
				wg.Done()
			}()
	...
	for range pws {
		if !tb.notifyBackgroundMergers() { // 唤醒  index merge 协程
			break
		}
	}
	tb.partsLock.Unlock()
...
func (tb *Table) notifyBackgroundMergers() bool { // 唤醒  index merge 协程
	select {
	case tb.needMergeCh <- struct{}{}: // 这里面有信号的时候，唤醒 merge 协程
		return true
	default:
		return false
	}
...
func (tb *Table) assistedMergeForFileParts() {  // 每次 inmemoryBlock 刷到磁盘，都会触发这里
	tb.partsLock.Lock()
	...
	err := tb.mergeExistingParts(false)  // part 文件 merge
	if err == nil {
		return
	}
...
func (tb *Table) createInmemoryPart(ibs []*inmemoryBlock) *partWrapper { // 一般把 15 个 inmomery block 进行 merge
	...
	bsrs := make([]*blockStreamReader, 0, len(ibs)) // inmomery block 包装成 blockStreamReader
	for _, ib := range ibs {
		if len(ib.items) == 0 {
			continue
		}
	...
	bsw := getBlockStreamWriter() // 写 part 文件的对象
	mpDst := &inmemoryPart{}
	...
	err := mergeBlockStreams(&mpDst.ph, bsw, bsrs, tb.prepareBlock, nil, &tb.inmemoryItemsMerged) // 这里做 inmemoryPart merge 操作
...
func (tb *Table) startMergeWorkers() {  // 启动多个 merge 协程
	// The actual number of concurrent merges is limited inside mergeWorker() below.
	for i := 0; i < cap(mergeWorkersLimitCh); i++ {  // n 个核就启动 n 个 merge 协程
		tb.wg.Add(1)
		go func() {
			tb.mergeWorker() // 启动 n 个 merge 协程
			tb.wg.Done()
		}()
	}
}
...
func getMaxInmemoryPartSize() uint64 {
	...
	n := uint64(0.05 * float64(memory.Allowed()) / maxInmemoryParts) // 假设 64gb 内存, 01GB
	if n < 1e6 {
		n = 1e6 // 100 万字节, 即 1mb
	}
	return n
...
func (tb *Table) mergeInmemoryParts() error {
	...
	tb.partsLock.Lock()
	pws := getPartsToMerge(tb.inmemoryParts, maxOutBytes, false) // 选择可以 merge 的块，并设置引用计数
	tb.partsLock.Unlock()

	return tb.mergeParts(pws, tb.stopCh, false) // 开始 merge 内存中的 part
}

func (tb *Table) mergeExistingParts(isFinal bool) error { // ??? 如何触发  final merge ?
	...
	maxOutBytes := tb.getMaxFilePartSize()  // 最大 372 GB

	tb.partsLock.Lock()
	dst := make([]*partWrapper, 0, len(tb.inmemoryParts)+len(tb.fileParts))
	dst = append(dst, tb.inmemoryParts...)
	dst = append(dst, tb.fileParts...)
	pws := getPartsToMerge(dst, maxOutBytes, isFinal)  // 所有的内存 part 和文件 part 都丢进去筛选
	tb.partsLock.Unlock()

	return tb.mergeParts(pws, tb.stopCh, isFinal)  // table 上的 merge 方法. 找到需要合并的 part 后，开始合并流程
}

func (tb *Table) mergeWorker() {  // 这是独立的 merge 携程
	...
	for {
		...
		err := tb.mergeExistingParts(isFinal)  // part 文件的 merge
		<-mergeWorkersLimitCh
		if err == nil {
		...
		if finalMergeDelaySeconds > 0 && fasttime.UnixTimestamp()-lastMergeTime > finalMergeDelaySeconds { // 每隔 N 秒就会 merge。但是，没有数据也就不会触发
		...
		case <-tb.stopCh:
			return
		case <-tb.needMergeCh: // 哪里触发了这个开关?  启动的时候触发一次 / 内存中 block 太多触发
		} // 没有事件，就会一直等待
	}
}
...
var finalMergeDelaySeconds = uint64(0) // 看起来会定期执行  final merge
...
func (tb *Table) mergeParts(pws []*partWrapper, stopCh <-chan struct{}, isFinal bool) error { // 选取好需要 merge 的  part 后，开始 merge
	if len(pws) == 0 {
		// Nothing to merge.
	...
	assertIsInMerge(pws)
	defer tb.releasePartsToMerge(pws) // 减少引用计数

	startTime := time.Now()

	// Initialize destination paths.
	dstPartType := getDstPartType(pws, isFinal)  // 看是文件 part 还是内存 part
	mergeIdx := tb.nextMergeIdx()  // 得到一个 unixnano 的递增 id
	dstPartPath := ""
	...
	bsrs := mustOpenBlockStreamReaders(pws) //根据文件 part 的路径，得到一堆 steam reader 对象
	...
	if dstPartType == partInmemory {
		...
		bsw.MustInitFromFilePart(dstPartPath, nocache, compressLevel)  // 在目的位置创建 part
	}

	// Merge source parts to destination part.
	ph, err := tb.mergePartsInternal(dstPartPath, bsw, bsrs, dstPartType, stopCh) // 这里做 file part merge
	putBlockStreamWriter(bsw)
	for _, bsr := range bsrs {
		putBlockStreamReader(bsr)
	}
	...
	dstSize := pDst.size
		// ??? merge 完成后，merge 前的 part 文件夹是怎么删除的?
	tb.swapSrcWithDstParts(pws, pwNew, dstPartType) // 把数据都写到新的 part 里，然后和这些 merge 后的 part 进行交换
...
func getDstPartType(pws []*partWrapper, isFinal bool) partType {
	...
	if isFinal || dstPartSize > getMaxInmemoryPartSize() {  // 当前文件数据要大于 1mb 才会进行 merge
		return partFile
	}
...
func mustOpenBlockStreamReaders(pws []*partWrapper) []*blockStreamReader {
	...
	for _, pw := range pws {
		...
		if pw.mp != nil {
			...
			bsr.MustInitFromFilePart(pw.p.path)  // 根据 path 来打开 part 文件夹
		}
		bsrs = append(bsrs, bsr)
	}
	return bsrs
...
func (tb *Table) mergePartsInternal(dstPartPath string, bsw *blockStreamWriter, bsrs []*blockStreamReader, dstPartType partType, stopCh <-chan struct{}) (*partHeader, error) { // file part 数据合并
	var ph partHeader
	...
	err := mergeBlockStreams(&ph, bsw, bsrs, tb.prepareBlock, stopCh, itemsMerged) // 这里做 file part 的合并
...
func (tb *Table) swapSrcWithDstParts(pws []*partWrapper, pwNew *partWrapper, dstPartType partType) {
	...
	tb.fileParts, removedFileParts = removeParts(tb.fileParts, m)  // 计算哪些 part 要被删除
	switch dstPartType {
	case partInmemory:
	...
	if removedFileParts > 0 || dstPartType == partFile {
		mustWritePartNames(tb.fileParts, tb.path)  // 把有效的 part name 写到文件夹
	}
	...
	for _, pw := range pws {
		atomic.StoreUint32(&pw.mustBeDeleted, 1)  // 原来在这里删除了 part 文件夹
		pw.decRef()
	}
}

func getPartsSize(pws []*partWrapper) uint64 { // 当前所有 part 文件夹加到一起，一共多少字节
	n := uint64(0)
	for _, pw := range pws {
		n += pw.p.size
	}
...
func (tb *Table) nextMergeIdx() uint64 {
	return atomic.AddUint64(&tb.mergeIdx, 1) // 从一个很早的毫秒数开始累加
}

var mergeWorkersLimitCh = make(chan struct{}, getWorkersCount())  // merge 协程的队列, n 个核就启动 n 个 merge 协程
...
func mustOpenParts(path string) []*partWrapper { // 打开  parts,  如果 parts 目录的内容为空，是可以正常打开的
	...
	partNames := mustReadPartNames(path) // 得到  parts 目录的名字. 没有 part.json，就会遍历目录
	...
	for _, partName := range partNames { // 逐个检查  parts 目录
	...
	// Open parts
	var pws []*partWrapper // part 这个层面的数据未排序
	for _, partName := range partNames {
		partPath := filepath.Join(path, partName)
		p := mustOpenFilePart(partPath) // 打开具体的 part 目录   /data/sharding-00-restore/indexdb/179D7BFA38672AE6/179DCB4A119F30CB/index.bin
		pw := &partWrapper{
			p:        p,
			refCount: 1,
		}
...
func mustWritePartNames(pws []*partWrapper, dstDir string) {  // 把有效的文件 part 的路径写入 parts.json
...
func mustReadPartNames(srcDir string) []string {
	partNamesPath := filepath.Join(srcDir, partsFilename) // 打开  parts.json
	if fs.IsPathExist(partNamesPath) {
...
func getPartsToMerge(pws []*partWrapper, maxOutBytes uint64, isFinal bool) []*partWrapper { // 选取可以 merge 的块
	pwsRemaining := make([]*partWrapper, 0, len(pws))
	for _, pw := range pws {
		if !pw.isInMerge {
			pwsRemaining = append(pwsRemaining, pw)  // 只选取没有在 merge 的块
		}
	}
	maxPartsToMerge := defaultPartsToMerge // 默认  15  个  part 文件
	var dst []*partWrapper
	if isFinal {
		...
		dst = appendPartsToMerge(dst[:0], pwsRemaining, maxPartsToMerge, maxOutBytes)  // 根据字节数来选择合并哪些不合并哪些
	}
	for _, pw := range dst {
		if pw.isInMerge {
			logger.Panicf("BUG: partWrapper.isInMerge is already set")
		}
		pw.isInMerge = true  // 把标志修改为正在 merge  //??? 并发问题怎么解决的
	}
	return dst
}
...
func appendPartsToMerge(dst, src []*partWrapper, maxPartsToMerge int, maxOutBytes uint64) []*partWrapper {  // 继续筛选适合 merge 的 part
	if len(src) < 2 {
	...
	maxInPartBytes := uint64(float64(maxOutBytes) / minMergeMultiplier)  // 如果某个 part 已经很大，也不要 merge 了
...
func removeParts(pws []*partWrapper, partsToRemove map[*partWrapper]bool) ([]*partWrapper, int) {  // 判断哪些 part 文件夹需要被删除
	dst := pws[:0]   //!!! 数组的边界处理得好巧妙啊 !!!
	for _, pw := range pws {
		if !partsToRemove[pw] {
			dst = append(dst, pw)
		}
