...
func (bsw *blockStreamWriter) MustInitFromFilePart(path string, nocache bool, compressLevel int) {  // 新建一个 part 文件夹
...
func (bsw *blockStreamWriter) MustClose() {
	...
	fs.MustWriteData(bsw.metaindexWriter, bsw.compressedMetaindexData)  // metaindex.bin
...
func (bsw *blockStreamWriter) WriteExternalBlock(b *Block, ph *partHeader, rowsMerged *uint64) {  // 当两个相同的 tsid 进行 merge 后，进入到写入的过程
	atomic.AddUint64(rowsMerged, uint64(b.rowsCount()))
	b.deduplicateSamplesDuringMerge()  // 对数据进行降采样
	headerData, timestampsData, valuesData := b.MarshalData(bsw.timestampsBlockOffset, bsw.valuesBlockOffset)  // 序列化
	usePrevTimestamps := len(bsw.prevTimestampsData) > 0 && bytes.Equal(timestampsData, bsw.prevTimestampsData)  // ??? 为什么要判断这个
	if usePrevTimestamps {
		...
		atomic.AddUint64(&timestampsBytesSaved, uint64(len(timestampsData)))  // !!! 神奇了，前一个 tsid 的时间序列和当前 tsid 的时间序列完全相同
	}
	...
	if !usePrevTimestamps {
		...
		fs.MustWriteData(bsw.timestampsWriter, timestampsData)  // timestamps.bin
		bsw.timestampsBlockOffset += uint64(len(timestampsData))
	}
	fs.MustWriteData(bsw.valuesWriter, valuesData)  // values.bin
...
func (bsw *blockStreamWriter) flushIndexData() {
	...
	fs.MustWriteData(bsw.indexWriter, bsw.compressedIndexData)  // index.bin
