...
type blockHeader struct {  // index.bin 中的数据格式
	...
	// in values file.
	ValuesBlockOffset uint64  // 指向 values.bin
...
var marshaledBlockHeaderSize = func() int {  // blockHeader 序列化后的字节数
	var bh blockHeader
	data := bh.Marshal(nil)
	return len(data)
}()
...
func (bh *blockHeader) Marshal(dst []byte) []byte {  // 序列化以后，89 字节
	dst = bh.TSID.Marshal(dst)  // 32 字节
	...
	dst = encoding.MarshalInt16(dst, bh.Scale)  // 86 字节
...
func (bh *blockHeader) Unmarshal(src []byte) ([]byte, error) {  // 结构是固定长度的
...
func unmarshalBlockHeaders(dst []blockHeader, src []byte, blockHeadersCount int) ([]blockHeader, error) {
	...
	// Verify that block headers are sorted by tsid.  // block 内的 tsid 可以是无序的。这个很好，方便追加
