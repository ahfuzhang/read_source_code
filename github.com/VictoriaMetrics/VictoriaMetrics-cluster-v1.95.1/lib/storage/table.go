...
func mustOpenTable(path string, s *Storage) *table {  // 加载数据部分
	...
	smallPartitionsPath := filepath.Join(path, smallDirname)  // small 文件夹不存在没关系
	...
	bigPartitionsPath := filepath.Join(path, bigDirname)  // big 文件夹不存在没关系
	...
	// Open partitions.
	pts := mustOpenPartitions(smallPartitionsPath, bigPartitionsPath, s)  // 打开大小分区
...
func (tb *table) MustAddRows(rows []rawRow) {  // 写入数据部分
	if len(rows) == 0 {
		return
	}
	...
	for i, ptw := range ptws {
		...
		ptw.pt.AddRows(rows)  // 在对应的 partition 写入
...
func (tb *table) getMinMaxTimestamps() (int64, int64) {  // 得到最大和最小的时间戳
...
func mustOpenPartitions(smallPartitionsPath, bigPartitionsPath string, s *Storage) []*partition {  // 打开大小分区
	...
	mustPopulatePartitionNames(smallPartitionsPath, ptNames)  // 遍历目录下的目录名，得到月份分区的文件夹列表
	mustPopulatePartitionNames(bigPartitionsPath, ptNames)
	var pts []*partition
	for ptName := range ptNames {  // 分区对象的数组
		smallPartsPath := filepath.Join(smallPartitionsPath, ptName)
		bigPartsPath := filepath.Join(bigPartitionsPath, ptName)
		pt := mustOpenPartition(smallPartsPath, bigPartsPath, s)  // 根据月分区名字，分别打开大小分区
		pts = append(pts, pt)
	}
	return pts
}

func mustPopulatePartitionNames(partitionsPath string, ptNames map[string]bool) {  // 读取月分区的名字   2023_12
