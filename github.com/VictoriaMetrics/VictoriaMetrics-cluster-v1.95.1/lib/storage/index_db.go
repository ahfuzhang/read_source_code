...
const (
	...
	// Prefix for Tag->MetricID entries.  // 索引的前缀
...
func mustOpenIndexDB(path string, s *Storage, isReadOnly *uint32) *indexDB { // 打开 indexdb/${cur}/ 数据库
	if s == nil {
		logger.Panicf("BUG: Storage must be nin-nil")
	}

	name := filepath.Base(path) // 返回最后一个路径名   179CACA495DAA352
	gen, err := strconv.ParseUint(name, 16, 64)
	if err != nil {
		logger.Panicf("FATAL: cannot parse indexdb path %q: %s", path, err)
	}
	// 猜测是根据目录下的  parts.json 文件来决定打开哪些目录的
...
type indexSearch struct {
	db *indexDB
	ts mergeset.TableSearch // 索引搜索对象
...
func generateTSID(dst *TSID, mn *MetricName) { // 产生新的 tsid
	...
	dst.MetricID = generateUniqueMetricID() // 产生 metric id
}
...
func (is *indexSearch) createGlobalIndexes(tsid *TSID, mn *MetricName) {
	...
	is.db.tb.AddItems(ii.Items) // 把索引格式化好，然后插入 table
}
...
func (is *indexSearch) searchTenantsOnDate(tenants map[string]struct{}, date uint64) error {
	...
	for ts.NextItem() {
		...
		is.projectID = projectID
		kb.B = is.marshalCommonPrefixForDate(kb.B[:0], date)
		ts.Seek(kb.B)  // ??? 不懂为什么这里要调用 seek 方法  // ??? 换了一个前缀来搜索吗?
	}
	if err := ts.Error(); err != nil {
...
// GetTSDBStatus returns topN entries for tsdb status for the given tfss, date and focusLabel.  // 得到 db 的状态
func (db *indexDB) GetTSDBStatus(qt *querytracer.Tracer, accountID, projectID uint32, tfss []*TagFilters, date uint64, focusLabel string, topN, maxMetrics int, deadline uint64) (*TSDBStatus, error) {
	...
	is := db.getIndexSearch(accountID, projectID, deadline) // 构造索引查询对象
...
// getTSDBStatus returns topN entries for tsdb status for the given tfss, date and focusLabel.  // 查询 tsdb 的状态
...
func (db *indexDB) searchMetricIDs(qt *querytracer.Tracer, tfss []*TagFilters, tr TimeRange, maxMetrics int, deadline uint64) ([]uint64, error) {
	...
	is := db.getIndexSearch(accountID, projectID, deadline)
	localMetricIDs, err := is.searchMetricIDs(qtChild, tfss, tr, maxMetrics) // 这里看起来从索引 1 搜索 tag
	db.putIndexSearch(is)
	if err != nil {
...
func (is *indexSearch) getTSIDByMetricNameNoExtDB(dst *TSID, metricName []byte, date uint64) bool {
	...
	kb.B = append(kb.B[:0], nsPrefixDateMetricNameToTSID)  // 搜索索引 7
	kb.B = encoding.MarshalUint64(kb.B, date)
...
func (is *indexSearch) searchMetricIDs(qt *querytracer.Tracer, tfss []*TagFilters, tr TimeRange, maxMetrics int) ([]uint64, error) {
	...
	metricIDs, err := is.searchMetricIDsInternal(qt, tfss, tr, maxMetrics) // 在索引 1 上搜索
	if err != nil {
		return nil, err
	}
	if metricIDs.Len() == 0 {
...
func (is *indexSearch) searchMetricIDsInternal(qt *querytracer.Tracer, tfss []*TagFilters, tr TimeRange, maxMetrics int) (*uint64set.Set, error) {
	...
	for _, tfs := range tfss {
		...
		err := is.updateMetricIDsForTagFilters(qtChild, metricIDs, tfs, tr, maxMetrics+1) // 通过索引 1 搜索
...
func (is *indexSearch) updateMetricIDsForTagFilters(qt *querytracer.Tracer, metricIDs *uint64set.Set, tfs *TagFilters, tr TimeRange, maxMetrics int) error {
	...
	m, err := is.getMetricIDsForDateAndFilters(qt, 0, tfs, maxMetrics) // 根据索引 1 搜索
	if err != nil {
		if errors.Is(err, errFallbackToGlobalSearch) {
...
func (is *indexSearch) getMetricIDsForTagFilter(qt *querytracer.Tracer, tf *tagFilter, maxMetrics int, maxLoopsCount int64) (*uint64set.Set, int64, error) {
	if tf.isNegative { // 根据 tf 中的索引前缀来搜索
...
func (is *indexSearch) getMetricIDsForTagFilterSlow(tf *tagFilter, f func(metricID uint64), maxLoopsCount int64) (int64, error) {
	...
	ts := &is.ts
	kb := &is.kb
	mp := &is.mp // tagToMetricIDsRowParser
	...
	for ts.NextItem() {
		...
		mp.ParseMetricIDs() // 解析索引尾部的一大堆 metric id
...
func (is *indexSearch) getMetricIDsForDateAndFilters(qt *querytracer.Tracer, date uint64, tfs *TagFilters, maxMetrics int) (*uint64set.Set, error) { // 根据索引 1 搜索
	if qt.Enabled() {
...
func (ii *indexItems) registerTagIndexes(prefix []byte, mn *MetricName, metricID uint64) { // 第一次添加这个索引的时候，明明只有一个 metric id
...
func (is *indexSearch) getMetricIDsForDateTagFilter(qt *querytracer.Tracer, tf *tagFilter, date uint64, commonPrefix []byte, // 根据标签，搜索 metric id
	...
	kb := kbPool.Get()
	defer kbPool.Put(kb)
	kb.B = is.marshalCommonPrefixForDate(kb.B[:0], date) // 构造索引前缀
	prefix := kb.B
	kb.B = append(kb.B, tf.prefix[len(commonPrefix):]...)
	tfNew := *tf
	tfNew.isNegative = false // isNegative for the original tf is handled by the caller.
	tfNew.prefix = kb.B
	metricIDs, loopsCount, err := is.getMetricIDsForTagFilter(qt, &tfNew, maxMetrics, maxLoopsCount) // 构造索引前缀后，进入搜索
	if err != nil {
...
func generateUniqueMetricID() uint64 { // 通过原子加来产生 metrics id
...
var nextUniqueMetricID = uint64(time.Now().UnixNano()) // metric id 的初始值是服务启动时候的时间戳
...
func (is *indexSearch) marshalCommonPrefixForDate(dst []byte, date uint64) []byte {
	if date == 0 {
		// Global index
		return is.marshalCommonPrefix(dst, nsPrefixTagToMetricIDs) // 不给日期，就在索引 1 上搜索
	}
	// Per-day index
	dst = is.marshalCommonPrefix(dst, nsPrefixDateTagToMetricIDs) // 有日期是索引 6
...
func (mp *tagToMetricIDsRowParser) ParseMetricIDs() { // 果然，索引的尾部就是一大堆的 metricid
