...
func (ib *inmemoryBlock) Less(i, j int) bool {
	...
	a := items[i]
	b := items[j]
	cpLen := uint32(len(ib.commonPrefix)) // 这里通过公共前缀，来减少字符串的比较字节数
	a.Start += cpLen
	b.Start += cpLen
	data := ib.data
...
type inmemoryBlock struct {
	...
	commonPrefix []byte // 这一项值是什么时候计算出来的???
...
func (ib *inmemoryBlock) SortItems() { // 对整个 momery block 的所有 string 进行排序
	if !ib.isSorted() {
...
func (ib *inmemoryBlock) updateCommonPrefixSorted() {
	...
	if len(items) > 1 {
		cpLen := commonPrefixLen(cp, items[len(items)-1].Bytes(data)) // 排序的情况下，只需要计算第一条和最后一条就行了
		cp = cp[:cpLen]
	}
	ib.commonPrefix = append(ib.commonPrefix[:0], cp...)
}

func (ib *inmemoryBlock) updateCommonPrefixUnsorted() { // 计算整个 block 中数据的公共前缀
	ib.commonPrefix = ib.commonPrefix[:0]
	items := ib.items
	if len(items) == 0 {
		return
...
func commonPrefixLen(a, b []byte) int { // todo: 这里可以修改为汇编实现
	i := 0
	if len(a) > len(b) {
		for i < len(b) && a[i] == b[i] {
			i++
...
func (ib *inmemoryBlock) Add(x []byte) bool { // 在内存 block 中插入一条数据
	data := ib.data
	if len(x)+len(data) > maxInmemoryBlockSize { // 超过 64 kb，返回 false
		return false
	}
	if cap(data) == 0 {
...
type storageBlock struct {
	itemsData []byte // items.bin 中的数据
	lensData  []byte // lens.bin 中的数据
}
...
func (ib *inmemoryBlock) MarshalSortedData(sb *storageBlock, firstItemDst, commonPrefixDst []byte, compressLevel int) ([]byte, []byte, uint32, marshalType) {
	...
	ib.updateCommonPrefixSorted()
	return ib.marshalData(sb, firstItemDst, commonPrefixDst, compressLevel) // 对数据进行压缩，变成了存储块对象
}
...
// Preconditions:   把内存 block 排序，计算公共前缀，计算与上一条的前缀，zstd 压缩
