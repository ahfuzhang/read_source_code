...
pub(crate) type BitMaskWord = u16;     // SIMD计算掩码后的结果，用一个16位的类型来存放
...
#[derive(Copy, Clone)]
pub(crate) struct Group(x86::__m128i);    //128位的值为一个Group，包含了16个控制字节

// FIXME: https://github.com/rust-lang/rust-clippy/issues/3859
#[allow(clippy::use_self)]
impl Group {
    /// Number of bytes in the group.
    pub(crate) const WIDTH: usize = mem::size_of::<Self>();    //这个值应该等于16
    ...
    pub(crate) const fn static_empty() -> &'static [u8; Group::WIDTH] {    //??? 看不懂
        #[repr(C)]
        struct AlignedBytes {
            _align: [Group; 0],
    ...
    pub(crate) unsafe fn load(ptr: *const u8) -> Self {
        Group(x86::_mm_loadu_si128(ptr.cast()))  // 加载 16 字节
    }
    ...
    pub(crate) unsafe fn load_aligned(ptr: *const u8) -> Self {    //rehash的时候使用，对齐的方式加载，应该性能更好
    ...
    #[inline]
    #[allow(clippy::cast_ptr_alignment)]
    pub(crate) unsafe fn store_aligned(self, ptr: *mut u8) {    //rehash的时候写回内存
        // FIXME: use align_offset once it stabilizes
        debug_assert_eq!(ptr as usize & (mem::align_of::<Self>() - 1), 0);
    ...
    /// the given value.
    #[inline]
    pub(crate) fn match_byte(self, byte: u8) -> BitMask {    //一次性匹配16字节，返回16个位
        #[allow(
