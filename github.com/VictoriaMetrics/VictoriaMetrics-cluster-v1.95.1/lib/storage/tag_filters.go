...
func convertToCompositeTagFilters(tfs *TagFilters) []*TagFilters {
	...
	for _, name := range names {
		...
		tfsCompiled := NewTagFilters(tfs.accountID, tfs.projectID)  // 创建一个 标签过滤器
...
func NewTagFilters(accountID, projectID uint32) *TagFilters {
	return &TagFilters{
		accountID:    accountID,
		projectID:    projectID,
		commonPrefix: marshalCommonPrefix(nil, nsPrefixTagToMetricIDs, accountID, projectID),  // 通过索引 1 来过滤标签
	}
}
