...
func (ps *partSearch) Init(p *part, tsids []TSID, tr TimeRange) {  // part 上的搜索对象
	ps.reset()
	ps.p = p  // part 对象
	...
	ps.tr = tr
	ps.metaindex = p.metaindex  // 元数据数组
...
func (ps *partSearch) NextBlock() bool {
	for {
		...
		if len(ps.bhs) == 0 {  // 一开始 bhs 是空的
			if !ps.nextBHS() {  // 读取下一个 block
				return false
			}
		}
		if ps.searchBHS() {
...
func (ps *partSearch) nextTSID() bool {  //  装载 tsid
	if ps.tsidIdx >= len(ps.tsids) {  // 所有要搜索的 tsid 都完成后，进入下一步
		ps.err = io.EOF
		return false
	}
	ps.BlockRef.bh.TSID = ps.tsids[ps.tsidIdx]  // blocker header 赋值为要搜索的第 n 个 tsid
	ps.tsidIdx++
	return true
}
...
func (ps *partSearch) nextBHS() bool {  // 读取下一个 block
	for len(ps.metaindex) > 0 {  // 遍历所有的元数据块
		...
		mr := &ps.metaindex[0]  // 如果这里可以 pop，证明进入 part search 的 tsid 数组是经过排序的
		ps.metaindex = ps.metaindex[1:]  // pop 掉第一个. 修改这里不会影响 part 对象的数组
		...
		if mr.MaxTimestamp < ps.tr.MinTimestamp {  // 这里的时间判断，在数据导入的时候肯定不需要
...
func (ps *partSearch) readIndexBlock(mr *metaindexRow) (*indexBlock, error) {// 根据元数据，读出一个 block
	ps.compressedIndexBuf = bytesutil.ResizeNoCopyMayOverallocate(ps.compressedIndexBuf, int(mr.IndexBlockSize))
	ps.p.indexFile.MustReadAt(ps.compressedIndexBuf, int64(mr.IndexBlockOffset))  // 读取 index.bin
        // 从 index.bin 中读取 IndexBlockOffset ~ IndexBlockSize
	var err error
	...
	ib := &indexBlock{}
	ib.bhs, err = unmarshalBlockHeaders(ib.bhs[:0], ps.indexBuf, int(mr.BlockHeadersCount))  // 读出来的数据是需要排序的
	if err != nil {
