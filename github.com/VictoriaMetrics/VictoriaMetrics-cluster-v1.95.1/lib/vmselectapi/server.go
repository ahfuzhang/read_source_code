...
func NewServer(addr string, api API, limits Limits, disableResponseCompression bool) (*Server, error) {  // 构造 storage 上的 api 对象
	...
	for {
		...
		go func() {
			...
			if err := s.processConn(bc); err != nil {
				...
				logger.Errorf("cannot process vmselect conn %s: %s", c.RemoteAddr(), err)  // 连接处理出错，肯定会打日志的
			}
		}()
	}
}
...
func (s *Server) processTSDBStatus(ctx *vmselectRequestCtx) error {
	...
	// Execute the request
	status, err := s.api.TSDBStatus(ctx.qt, &ctx.sq, focusLabel, int(topN), ctx.deadline)  // 查询 db 状态
	if err != nil {
		return ctx.writeErrorMessage(err)
	}
...
func (s *Server) processSearch(ctx *vmselectRequestCtx) error {
	...
	blocksRead := 0
	for bi.NextBlock(&ctx.mb) {  // 加载文件的数据到 block 对象
		blocksRead++
		s.metricBlocksRead.Inc()
		s.metricRowsRead.Add(ctx.mb.Block.RowsCount())

		ctx.dataBuf = ctx.mb.Marshal(ctx.dataBuf[:0])  // 返回给 vm-select 的时候，在这里序列化
