...
fn h2(hash: u64) -> u8 {  // 取hashcode的高7位
    ...
    // So we use MIN_HASH_LEN constant to handle this.   //x64下面肯定是8
...
impl TableLayout {
    #[inline]
    const fn new<T>() -> Self {  //T是 (K,V)， 就是一个slot的size
        let layout = Layout::new::<T>();   //??? Layout哪儿来的?
        Self {
            size: layout.size(),
            ctrl_align: if layout.align() > Group::WIDTH {  //假设是按照16字节对齐
                layout.align()    //假设 layout.size() = 15
            } else {
                Group::WIDTH
            },
        }
...
impl<T> Bucket<T> {
    ...
    /// [`Eq`]: https://doc.rust-lang.org/core/cmp/trait.Eq.html
    #[inline]
    pub(crate) unsafe fn write(&self, val: T) {  //在桶内写入数据
...
struct RawTableInner {  //看起来是内部最核心的实现
...
impl<T, A: Allocator> RawTable<T, A> {
    ...
    #[cfg_attr(feature = "inline-more", inline)]
    unsafe fn erase_no_drop(&mut self, item: &Bucket<T>) {   //找到以后执行删除
    ...
    #[cfg_attr(feature = "inline-more", inline)]
    #[allow(clippy::needless_pass_by_value)]
    pub unsafe fn remove(&mut self, item: Bucket<T>) -> (T, InsertSlot) {   //查找到以后，执行删除
        self.erase_no_drop(&item);
        (
            item.read(),
            InsertSlot {
    ...
    pub fn reserve(&mut self, additional: usize, hasher: impl Fn(&T) -> u64) {
        if unlikely(additional > self.table.growth_left) {   //additional一般是1
            // Avoid `Result::unwrap_or_else` because it bloats LLVM IR.
            unsafe {
                    ...
                    .is_err()
                {
                    ...
                    hint::unreachable_unchecked()   //这个啥意思?
                }
            }
        }
    }
    ...
    #[cold]
    #[inline(never)]
    unsafe fn reserve_rehash(   //没有空间的时候执行
        &mut self,
        additional: usize,
        hasher: impl Fn(&T) -> u64,
        fallibility: Fallibility,
        ...
        unsafe {
            ...
            self.table.reserve_rehash_inner(
                ...
                fallibility,
                Self::TABLE_LAYOUT,   //得到sizeof字段的值
                if T::NEEDS_DROP {   //用于析构对象的工具
    ...
    pub fn insert(&mut self, hash: u64, value: T, hasher: impl Fn(&T) -> u64) -> Bucket<T> {
        unsafe {
            ...
            let mut slot = self.table.find_insert_slot(hash);   //寻找插入位置
            ...
            if unlikely(self.table.growth_left == 0 && special_is_empty(old_ctrl)) {   //如果达到了该扩容的情况，先扩容
                ...
                slot = self.table.find_insert_slot(hash);   //如果没有空间了，但是当前又是EMPTY的标志，那就rehash
            }

            self.insert_in_slot(hash, slot, value)  //定位到桶  在桶内写回数据  返回桶
        }
    }
...
impl RawTableInner {
    ...
    #[cfg_attr(feature = "inline-more", inline)]
    unsafe fn new_uninitialized<A>(  //构造一个table
        alloc: &A,
    ...
    #[inline]
    fn fallible_with_capacity<A>(   //构造一个新的table，一般在resize的时候调用
        alloc: &A,   //传入了一个分配器
    ...
    unsafe fn find_insert_slot(&self, hash: u64) -> InsertSlot {
        ...
        loop {
            ...
            //   always return "0" (zero), so Group::load will read unaligned `Group::static_empty()`
            //   bytes, which is safe (see RawTableInner::new).
            let group = unsafe { Group::load(self.ctrl(probe_seq.pos)) };  //当这一字节处于整个ctrl数组的边缘的时候，就必须在最后加一个Group，以此避免溢出   //加载当前Group

            let index = self.find_insert_slot_in_group(&group, &probe_seq);
            if likely(index.is_some()) {  //当前group找个空位
                // SAFETY:
                // * Caller of this function ensures that the control bytes are properly initialized.
                //
                // * We use this function with the slot / index found by `self.find_insert_slot_in_group`
    ...
    unsafe fn prepare_rehash_in_place(&mut self) {
        ...
        if unlikely(self.buckets() < Group::WIDTH) {  //复制到溢出区
    ...
    unsafe fn record_item_insert_at(&mut self, index: usize, old_ctrl: u8, hash: u64) {
        self.growth_left -= usize::from(special_is_empty(old_ctrl));  //修改剩余空间  EXMPTY标志不减少剩余空间
    ...
    #[allow(clippy::mut_mut)]
    #[inline]
    fn prepare_resize<'a, A>(   //resize的时候调用此函数
        &self,
        alloc: &'a A,
        table_layout: TableLayout,
        capacity: usize,
    ...
    #[inline(always)]
    unsafe fn reserve_rehash_inner<A>(   //负载满，调用此函数
        &mut self,
        alloc: &A,
    ...
    #[allow(clippy::inline_always)]
    #[inline(always)]
    unsafe fn resize_inner<A>(     //缩容(或者扩容)，新分配一个hash表，然后把元素拷贝过去。
        &mut self,
        alloc: &A,
        capacity: usize,
        hasher: &dyn Fn(&mut Self, usize) -> u64,
        ...
        A: Allocator,
    {
        ...
        for full_byte_index in self.full_buckets_indices() {     //遍历旧的table  //todo: 逐个找是不是太慢了
            // This may panic.
            let hash = hasher(self, full_byte_index);     //重新计算hashcode
    ...
    unsafe fn rehash_in_place(
        ...
        'outer: for i in 0..guard.buckets() {//遍历控制字段
            ...
            'inner: loop {
                ...
                let new_i = guard.find_insert_slot(hash).index;      //在一个假定是空的位置，重新寻找新的插入点
    ...
    unsafe fn erase(&mut self, index: usize) {     //根据下标进行删除
        ...
        // `trailing_zeros` refers to the bytes at the beginning of a group.      //前一个group和后一个group的间隔超过了16
...
pub(crate) struct RawIterRange<T> {
    ...
    current_group: BitMaskIter,     //当前组的16bit的使用情况，第一次指向第0个group
...
impl<T> Iterator for RawIterRange<T> {
    ...
    #[cfg_attr(feature = "inline-more", inline)]
    fn next(&mut self) -> Option<Bucket<T>> {  //先找第一个不为0的bit  
        unsafe {
