...
func GetCommonParams(r *http.Request) (*CommonParams, error) {
	// Extract tenantID
	tenantID, err := logstorage.GetTenantIDFromRequest(r)  // 从 http header 传递租户的信息
	if err != nil {
		return nil, err
	}
	...
	var timeField = "_time"
	if tf := r.FormValue("_time_field"); tf != "" {  // ??? 没看懂参数怎么传进来的
		timeField = tf
	}
	...
	streamFields := httputils.GetArray(r, "_stream_fields")  // 猜测可以通过 url 参数来设定要哪些字段和不要哪些字段
