...
type eventLoop struct {  // 实际的事件循环对象
	sync.Mutex
	opts *options
	svr  *server
	stop chan error
...
// Serve implements EventLoop.  // 开启事件循环，程序在这里卡住
func (evl *eventLoop) Serve(ln net.Listener) error {
	npln, err := ConvertListener(ln)  // 把系统的 net.Listener 包一层封装
	if err != nil {
		return err
	}
	evl.Lock()
	evl.svr = newServer(npln, evl.opts, evl.quit)  // 创建一个 server 对象
	evl.svr.Run()  // 执行 server 对象的 run. 
	evl.Unlock()

	err = evl.waitQuit()  // 程序会在这里卡住
