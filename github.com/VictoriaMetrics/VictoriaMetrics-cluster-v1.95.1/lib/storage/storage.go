...
type Storage struct {
	...
	idbCurr atomic.Pointer[indexDB]  // 当前分区
	...
	isReadOnly uint32  // 磁盘空间不够的时候就会 readonly
}
...
func MustOpenStorage(path string, retention time.Duration, maxHourlySeries, maxDailySeries int) *Storage { // 服务启动的时候，打开 /data 目录
	...
	s := &Storage{
		path:           path,
		cachePath:      filepath.Join(path, cacheDirname), // ??? cache 目录是啥
	...
	s.flockF = fs.MustCreateFlockFile(path) // 创建  flock.lock

	// Check whether restore process finished successfully
	restoreLockF := filepath.Join(path, backupnames.RestoreInProgressFilename) // 当前目录如果出现在恢复的信息，说明备份不完整
	...
	// Load caches.
	mem := memory.Allowed()
	s.tsidCache = s.mustLoadCache("metricName_tsid", getTSIDCacheSize()) // 设置各种关键的 cache
	...
	// Load metadata
	metadataDir := filepath.Join(path, metadataDirname) // 加载  /data/metadata,  就只有一个时间戳文件而已
	isEmptyDB := !fs.IsPathExist(filepath.Join(path, indexdbDirname))
	fs.MustMkdirIfNotExist(metadataDir)
	s.minTimestampForCompositeIndex = mustGetMinTimestampForCompositeIndex(metadataDir, isEmptyDB) // 加载或者创建一个最小时间戳
	...
	idbNext, idbCurr, idbPrev := s.mustOpenIndexDBTables(idbPath) // 打开  indexdb，仅 prev 和 curr 两个分区有效

	idbCurr.SetExtDB(idbPrev)  // 设定前一个搜索 db
	idbNext.SetExtDB(idbCurr)

	s.idbCurr.Store(idbCurr)  // !!! 这里，这里不存在 idbPrev
	...
	// Load data
	tablePath := filepath.Join(path, dataDirname) // 数据部分
	tb := mustOpenTable(tablePath, s)  // 打开数据表
	s.tb = tb
...
func (s *Storage) mustRotateIndexDB(currentTime time.Time) { // index db 的切换
...
func mustGetMinTimestampForCompositeIndex(metadataDir string, isEmptyDB bool) int64 {
	...
	if err == nil {
		return minTimestamp // 如果有最小时间戳就返回
	} // 以下创造一个最小时间戳
...
func loadMinTimestampForCompositeIndex(path string) (int64, error) { // 加载文件 /data/metadata/minTimestampForCompositeIndex
...
func (s *Storage) SearchMetricNames(qt *querytracer.Tracer, tfss []*TagFilters, tr TimeRange, maxMetrics int, deadline uint64) ([]string, error) {
	...
	defer qt.Done()
	metricIDs, err := s.idb().searchMetricIDs(qt, tfss, tr, maxMetrics, deadline)  // 搜索，然后返回一堆 metric id
	if err != nil {
		return nil, err
	}
...
// GetTSDBStatus returns TSDB status data for /api/v1/status/tsdb    // 读取 tsdb 的状态信息
...
func (s *Storage) AddRows(mrs []MetricRow, precisionBits uint8) error { // vm-insert 的数据，通过这个方法写入
	if len(mrs) == 0 {
		return nil
	}
...
func (s *Storage) add(rows []rawRow, dstMrs []*MetricRow, mrs []MetricRow, precisionBits uint8) error { // 来自 vm-insert 的数据，批量写入
	idb := s.idb()
	generation := idb.generation
	is := idb.getIndexSearch(0, 0, noDeadline) // 看起来是查询对象
	defer idb.putIndexSearch(is)
	...
	minTimestamp, maxTimestamp := s.tb.getMinMaxTimestamps() // 得到允许的时间范围
	...
	for i := range mrs {
		...
		if mr.Timestamp < minTimestamp {
			...
			atomic.AddUint64(&s.tooSmallTimestampRows, 1) // 对  counter 值进行累加
			continue
		}
	...
	if err != nil {
		...
		s.tb.MustAddRows(rows)  // 猜测是写入数据部分
	}
	if err != nil {
...
func createAllIndexesForMetricName(is *indexSearch, mn *MetricName, tsid *TSID, date uint64) { // 创建全新的 time series
...
// GetIndexDBTableNames 得到索引 partition 的名字
...
func (s *Storage) mustOpenIndexDBTables(path string) (next, curr, prev *indexDB) { // 加载  indexdb
...
var indexDBTableNameRegexp = regexp.MustCompile("^[0-9A-F]{16}$") // indexdb 中，索引目录的格式
