...
type BlockRef struct {  // 代表数据部分的当前游标
	p  *part
	bh blockHeader  // 指向当前游标的 block,
}
...
func (br *BlockRef) MustReadBlock(dst *Block) {  // 这个读取块  根据 blockHeader 的信息，把数据读取到 Block 对象的 buffer 中
	dst.Reset()
	dst.bh = br.bh
...
type MetricBlock struct {  // 最终返回给 vm-select 的格式  metric -> data
...
func (mb *MetricBlock) Marshal(dst []byte) []byte {  // 返回给 select 的时候，在这里序列化
...
func MarshalBlock(dst []byte, b *Block) []byte {
	...
	dst = encoding.MarshalBytes(dst, b.timestampsData)  // 把未 decode 的数据直接发送给 vm-select
...
func (s *Search) Init(qt *querytracer.Tracer, storage *Storage, tfss []*TagFilters, tr TimeRange, maxMetrics int, deadline uint64) int {
	...
	s.ts.Init(storage.tb, tsids, tr)  // 数据部分的搜索对象
...
func (s *Search) NextMetricBlock() bool {  // 搜索对象中，获得下一个 metric -> data 的组合
	if s.err != nil {
		return false
	}
	for s.ts.NextBlock() {  // 遍历数据块   猜测是把找到的数据放在了 ts.BlockRef
		...
		s.loops++
		tsid := &s.ts.BlockRef.bh.TSID  // 通过游标读 tsid
...
type SearchQuery struct {  // 查询过程中使用的对象
	AccountID uint32
	ProjectID uint32
