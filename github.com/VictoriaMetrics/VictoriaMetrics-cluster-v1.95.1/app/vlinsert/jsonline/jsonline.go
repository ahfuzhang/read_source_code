...
func RequestHandler(w http.ResponseWriter, r *http.Request) bool { //  /insert/jsonline
	startTime := time.Now()
	...
	cp, err := insertutils.GetCommonParams(r)  // 从 header , querystring 中获取相关字段
	if err != nil {
		httpserver.Errorf(w, r, "%s", err)
		return true
	}
	if err := vlstorage.CanWriteData(); err != nil {  // 判断存储是不是只读
		httpserver.Errorf(w, r, "%s", err)
		return true
	}
	lr := logstorage.GetLogRows(cp.StreamFields, cp.IgnoreFields)  // 每次请求都要为数据治理的字段进行初始化
	...
	n := 0
	for {
		ok, err := readLine(sc, cp.TimeField, cp.MsgField, processLogMessage)  // 逐行读取
		wcr.DecConcurrency()  // 这里的并发管理相比 vm-storage 太弱了
		if err != nil {
	...
	vlstorage.MustAddRows(lr)  // 插入到 storage 中
