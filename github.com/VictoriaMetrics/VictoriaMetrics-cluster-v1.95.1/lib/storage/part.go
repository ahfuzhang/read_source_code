...
type part struct {
	ph partHeader  // 数据来自 metadata.json
	...
	timestampsFile fs.MustReadAtCloser  // timestamps.bin
	valuesFile     fs.MustReadAtCloser  // values.bin
	indexFile      fs.MustReadAtCloser  // index.bin

	metaindex []metaindexRow  // 数据来自 metaindex.bin
}

// mustOpenFilePart opens file-based part from the given path.
func mustOpenFilePart(path string) *part {  // 打开具体的 part 目录
	...
	metaindexFile := filestream.MustOpen(metaindexPath, true)  // metaindex.bin
...
type indexBlock struct {
	bhs []blockHeader  // 来自 index.bin, blockHeader数组
}  // 数组的每一项是一个独立的 tsid
