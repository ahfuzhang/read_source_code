...
func (ps *partSearch) Init(p *part) {  // 需要搞清楚搜索是如何进行的
	ps.reset()
...
func (ps *partSearch) tryFastSeek(k []byte) bool {  // 从当前的 inmemory block 中搜索
	if ps.ib == nil {
		return false
	}
	items := ps.ib.items
...
func (ps *partSearch) readInmemoryBlock(bh *blockHeader) (*inmemoryBlock, error) {
	...
	ps.p.itemsFile.MustReadAt(ps.sb.itemsData, int64(bh.itemsBlockOffset))  // 读文件内容
