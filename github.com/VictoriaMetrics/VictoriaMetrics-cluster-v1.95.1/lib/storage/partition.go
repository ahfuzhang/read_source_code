...
type partition struct {
	...
	mergeIdx uint64  // 一个 unix nano 值，被用于做递增 id
...
func (pt *partition) startBackgroundWorkers() {  // 各种协程
...
func mustOpenPartition(smallPartsPath, bigPartsPath string, s *Storage) *partition { // 根据月分区名字，分别打开大小分区
	smallPartsPath = filepath.Clean(smallPartsPath)
	bigPartsPath = filepath.Clean(bigPartsPath)

	name := filepath.Base(smallPartsPath)  // 月份的名字
	if !strings.HasSuffix(bigPartsPath, name) { // 大分区中一定要存在小分区的月份目录，否则就出错
	...
	partNamesSmall, partNamesBig := mustReadPartNames(smallPartsPath, bigPartsPath)  // 读取 part 目录的名字

	smallParts := mustOpenParts(smallPartsPath, partNamesSmall)  // 根据名字打开 part 文件
	bigParts := mustOpenParts(bigPartsPath, partNamesBig)  // 返回了 part 对象的数组
	...
	pt := newPartition(name, smallPartsPath, bigPartsPath, s)// 分区对象
	pt.smallParts = smallParts  // 把  parts 数组放上去
...
func (rrss *rawRowsShards) addRows(pt *partition, rows []rawRow) {  // 在内存表中写入
	shards := rrss.shards
...
func (rrs *rawRowsShard) addRows(pt *partition, rows []rawRow) []rawRow {  // 内存表中写入数据
...
func (pt *partition) flushRowsToParts(rows []rawRow) {  // 把内存表的数据刷到磁盘
	if len(rows) == 0 {
		return
	}
...
func (pt *partition) createInmemoryPart(rows []rawRow) *partWrapper {  // 把原始行加成一个 内存 part
	if len(rows) == 0 {
		return nil
	}
	mp := getInmemoryPart()
...
func assertIsInMerge(pws []*partWrapper) {  // 检查是否已经在 merge
	for _, pw := range pws {
		if !pw.isInMerge {
...
func (pt *partition) mergeParts(pws []*partWrapper, stopCh <-chan struct{}, isFinal bool) error {  // 分区上的 merge 方法
	if len(pws) == 0 {
		// Nothing to merge.
		return errNothingToMerge
	}

	assertIsInMerge(pws)  // 检查是否已经在 merge
	...
	dstPartPath := pt.getDstPartPath(dstPartType, mergeIdx)  // merge 后放到一个新的 part 目录
	...
	bsrs := mustOpenBlockStreamReaders(pws)   // 从 part 数组，得到 part 的读对象
	...
	bsw := getBlockStreamWriter()  // 数据流的写对象
	...
	if dstPartType == partInmemory {
		...
		bsw.MustInitFromFilePart(dstPartPath, nocache, compressLevel)  // 在新的路径生成 part 写对象
	}

	// Merge source parts to destination part.
	ph, err := pt.mergePartsInternal(dstPartPath, bsw, bsrs, dstPartType, stopCh)  // merge 的主流程
...
func mustOpenBlockStreamReaders(pws []*partWrapper) []*blockStreamReader {  // 从 part 数组，得到 part 的读对象
...
func (pt *partition) mergePartsInternal(dstPartPath string, bsw *blockStreamWriter, bsrs []*blockStreamReader, dstPartType partType, stopCh <-chan struct{}) (*partHeader, error) {
	...
	retentionDeadline := timestampFromTime(time.Now()) - pt.s.retentionMsecs  // 计算出最小时间
	atomic.AddUint64(activeMerges, 1)
	err := mergeBlockStreams(&ph, bsw, bsrs, stopCh, pt.s, retentionDeadline, rowsMerged, rowsDeleted)  // 数据 merge 主流程
	atomic.AddUint64(activeMerges, ^uint64(0))  // 很酷，减 1 操作
	...
	if dstPartPath != "" {
		...
		ph.MustWriteMetadata(dstPartPath)  // 写入 metadata.json
	}
	return &ph, nil
}
...
func (pt *partition) nextMergeIdx() uint64 {  // 得到一个递增的 id
...
func mustOpenParts(path string, partNames []string) []*partWrapper {  // 根据名字打开 part 数据文件, part 目录为空是没关系的
	...
	for _, de := range des {
		...
		if _, ok := m[fn]; !ok {
			...
			fs.MustRemoveAll(deletePath)  // 打开 part 目录的时候，会排他性的删除其他目录
		}
	}
	...
	for _, partName := range partNames {
		partPath := filepath.Join(path, partName)
		p := mustOpenFilePart(partPath)  // 打开具体的  part 目录，返回 part 对象
		pw := &partWrapper{
			p:        p,
			refCount: 1,
		}
...
func mustReadPartNames(smallPartsPath, bigPartsPath string) ([]string, []string) {
	partNamesPath := filepath.Join(smallPartsPath, partsFilename)
	if fs.IsPathExist(partNamesPath) {
		data, err := os.ReadFile(partNamesPath) // 读取 ${root}/data/small/${year}${month}/parts.json
		if err != nil {
			logger.Panicf("FATAL: cannot read %s file: %s", partsFilename, err)
		}
		var partNames partNamesJSON
		if err := json.Unmarshal(data, &partNames); err != nil { // json 格式： {"Small":["179DCFFD27D69D14"],"Big":[]}
	...
	partNamesSmall := mustReadPartNamesFromDir(smallPartsPath) // json 文件不存在的时候，扫描目录
	partNamesBig := mustReadPartNamesFromDir(bigPartsPath)
	return partNamesSmall, partNamesBig
}

func mustReadPartNamesFromDir(srcDir string) []string { // 遍历  part 文件夹
	...
	for _, de := range des {
		...
		partNames = append(partNames, partName) // 把分区名加进去
	}
	return partNames
}
