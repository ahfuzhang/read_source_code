...
type TableSearch struct {
	...
	tb *Table  // 对 table 对象的引用

	pws []*partWrapper  // 主要是需要这里的 part
...
func (ts *TableSearch) Init(tb *Table) {  // 通过 part 对象的数组来初始化 part
	if ts.needClosing {
	...
	ts.pws = ts.tb.getParts(ts.pws[:0])  // 引用计数
	...
	for i, pw := range ts.pws {
		ts.psPool[i].Init(pw.p)  // partSearch 对象，与  part 对象的数量一致
	}
}
...
func (ts *TableSearch) FirstItemWithPrefix(prefix []byte) error {  // 快速判断一条记录是否存在
	ts.Seek(prefix)
	if !ts.NextItem() {
		if err := ts.Error(); err != nil {
			return err
