...
type rawRow struct {  // 即将要写入数据部分的格式
...
func (rrm *rawRowsMarshaler) marshalToInmemoryPart(mp *inmemoryPart, rows []rawRow) {  // 对原始 row 进行序列化
	if len(rows) == 0 {
		return
	}
	...
	for i := range rows {
		r = &rows[i]
		if r.TSID.MetricID == tsid.MetricID && len(rrm.auxTimestamps) < maxRowsPerBlock {  // 相同的 tsid，只是追加
		...
		tmpBlock.Init(tsid, rrm.auxTimestamps, rrm.auxValues, scale, precisionBits)  // 不同的 tsid, 初始化为新的 block
		rrm.bsw.WriteExternalBlock(tmpBlock, ph, &rowsMerged)  // ??? 到底是怎么写进去的呢，没看明白
