...
type blockStreamReader struct {  // merge 的时候，使用这个来读索引
	// Block contains the current block if Next returned true.
	Block inmemoryBlock  // 当前读出的块放在这里
...
func (bsr *blockStreamReader) MustInitFromFilePart(path string) {  // 从 part 文件夹打开数据
	bsr.reset()
...
func (bsr *blockStreamReader) CurrItem() string {  // 已经加载到内存中的数据，取当前一条
