...
// http://stackoverflow.com/questions/32630458/setting-m256i-to-the-value-of-two-m128i-values
...
void encode(LOOKUP_FN lookup, const uint8_t *input, size_t bytes,
            uint8_t *output) {
  ...
  for (size_t i = 0; i < bytes; i += 2 * 4 * 3) {
    ...
    // in = [0HHH|0GGG|0FFF|0EEE[0DDD|0CCC|0BBB|0AAA]  // 把  24  字节扩展为 32  字节
    // 4字节为一组，一共  8  组
    const __m256i shuf =
    ...
    const __m256i shuf =
        _mm256_set_epi8(10, 11, 9, 10, 7, 8, 6, 7, 4, 5, 3, 4, 1, 2, 0, 1,
                        ...
                        14, 15, 13, 14, 11, 12, 10, 11, 8, 9, 7, 8, 5, 6, 4, 4);

    __m256i in = _mm256_shuffle_epi8(data, shuf);
#endif

    // this part is well commented in encode.sse.cpp
    // 到这里的数据格式为： in = [0HHH|0GGG|0FFF|0EEE[0DDD|0CCC|0BBB|0AAA]
    // 0x0fc0fc00 从  32 位扩展到  256  位
    //  00001111_11000000_11111100_00000000
    const __m256i t0 = _mm256_and_si256(in, _mm256_set1_epi32(0x0fc0fc00));
    // 0x003f03f0 => 00000000_00111111_00000011_11110000
    // _mm256_mulhi_epu16 是十六位的乘法  高 16 位为结果
    //   2 字节 * 16 组
    const __m256i t1 = _mm256_mulhi_epu16(t0, _mm256_set1_epi32(0x04000040));

    // 0x003f03f0 => 00000000_00111111_00000011_11110000
    const __m256i t2 = _mm256_and_si256(in, _mm256_set1_epi32(0x003f03f0));
    // 0x01000010 => 00000001_00000000_00000000_00010000
    // _mm256_mullo_epi16, 16 位乘法，低 16 位为结果
    const __m256i t3 = _mm256_mullo_epi16(t2, _mm256_set1_epi32(0x01000010));

    // 两种计算方法的结果，再做 or 运算
    const __m256i indices = _mm256_or_si256(t1, t3);

    const auto result = lookup(indices);  // 查表
...
template <typename LOOKUP_FN>
void encode_unrolled(LOOKUP_FN lookup, const uint8_t *input, size_t bytes,
                     uint8_t *output) {
...
template <typename LOOKUP_FN>
void encode_bmi2(LOOKUP_FN lookup, const uint8_t *input, size_t bytes,
                 uint8_t *output) {
  ...
  for (size_t i = 0; i < bytes; i += 4 * 6) {
    ...
    const __m256i shuf = _mm256_set_epi8(
        -1, -1, 9, 10, 11, 6, 7, 8, -1, -1, 3, 4, 5, 0, 1, 2,

        -1, -1, 13, 14, 15, 10, 11, 12, -1, -1, 7, 8, 9, 4, 5, 6);
    ...
    const uint64_t expanded_0 =
        _pdep_u64(_mm_extract_epi64(lo, 0), 0x3f3f3f3f3f3f3f3flu);
    const uint64_t expanded_1 =
        _pdep_u64(_mm_extract_epi64(lo, 1), 0x3f3f3f3f3f3f3f3flu);
    const uint64_t expanded_2 =
        _pdep_u64(_mm_extract_epi64(hi, 0), 0x3f3f3f3f3f3f3f3flu);
    const uint64_t expanded_3 =
        _pdep_u64(_mm_extract_epi64(hi, 1), 0x3f3f3f3f3f3f3f3flu);

    const __m256i indices =
        _mm256_setr_epi64x(expanded_0, expanded_1, expanded_2, expanded_3);
