...
// using the given handler.
func ListenAndServe(addr string, handler RequestHandler) error { // 用户使用的入口
	s := &Server{
		Handler: handler,
	}
...
type Server struct {
	...
	Handler RequestHandler // 当请求到达时候的回调
...
func (s *Server) ListenAndServe(addr string) error { // 监听某个地址，并开始服务
...
//
// Serve blocks until the given listener returns permanent error.
func (s *Server) Serve(ln net.Listener) error { // 用户使用的入口
	...
	wp := &workerPool{
		WorkerFunc:            s.serveConn, // 这个回调负责处理新用户的连接
...
func (s *Server) serveConn(c net.Conn) (err error) { // 负责处理新用户的连接
	...
	for { // 处理整个连接的生命周期
		...
		if hijackHandler != nil {
			...
			go hijackConnHandler(ctx, hjr, c, s, hijackHandler) // 加了劫持，就会每次都 go 出来
			err = errHijacked
			break
		}
...
func (s *Server) acquireCtx(c net.Conn) (ctx *RequestCtx) { // 从内存池获取 context
	v := s.ctxPool.Get()
	if v == nil {
