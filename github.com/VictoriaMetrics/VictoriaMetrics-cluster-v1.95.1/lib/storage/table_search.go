...
type tableSearch struct {  // 用于数据部分的搜索
	BlockRef *BlockRef  // 这个是当前游标  猜测每次搜索的数据，放在这个结构里
...
func (ts *tableSearch) Init(tb *table, tsids []TSID, tr TimeRange) {  // 根据一堆 tsids 来找对应的 value 和 timestamp
	if ts.needClosing {
	...
	ts.ptws = tb.GetPartitions(ts.ptws[:0])  // 对分区进行引用计数
	...
	ts.ptsHeap = ts.ptsHeap[:0]  // 使用二叉堆来构造火山模型的搜索
	for i := range ts.ptsPool {
		pts := &ts.ptsPool[i]
		if !pts.NextBlock() {
...
func (ts *tableSearch) NextBlock() bool {  // 逐个 block 读取。每个 block 就是一个独立的 tsid 吗?
	if ts.err != nil {
		return false
	}
	if ts.nextBlockNoop {
