...
type blockStreamReader struct {
	...
	indexBlockOffset      uint64  // 在 index.bin 中读到哪个位置了

	prevTimestampsBlockOffset uint64
	prevTimestampsData        []byte

	indexData           []byte   // 当前 metaindexRow 中对应的 index.bin 中的数据
	compressedIndexData []byte

	// Cursor to indexData.
	indexCursor []byte   // 当前 metaindexRow 中对应的 index.bin 中的数据  // ??? 看不懂
...
func (bsr *blockStreamReader) MustInitFromFilePart(path string) {  // 从 part 文件夹打开一个读对象
	bsr.reset()
...
func (bsr *blockStreamReader) NextBlock() bool {  // 逐个 block 读取
	if bsr.err != nil {
		return false
	}
	bsr.tsidPrev = bsr.Block.bh.TSID  // 第一次，这个值是空的
	bsr.Block.Reset()
	err := bsr.readBlock()  // 一个 block 就是一个 tsid， block 的数据来自 values.bin 和 timestamps.bin，但是未 decode
	if err == nil {
...
func (bsr *blockStreamReader) readBlock() error {  // 把 timestamps.bin 和 values.bin 中的数据，读到 block 中
	if len(bsr.indexCursor) == 0 {  // 还没有从 index.bin 读数据
		...
		bsr.indexBlockHeadersCount = 0
		if err := bsr.readIndexBlock(); err != nil {  // index.bin 的数据还没读的时候，这里读
			if err == io.EOF {
				return io.EOF
			}
	...
	// Read block header.
	if len(bsr.indexCursor) < marshaledBlockHeaderSize {  // 看起来每个块都是固定的 89 字节
	...
	bsr.Block.headerData = append(bsr.Block.headerData[:0], bsr.indexCursor[:marshaledBlockHeaderSize]...)  // 拷贝 89 字节
	bsr.indexCursor = bsr.indexCursor[marshaledBlockHeaderSize:]  // 消费了第一个
	tail, err := bsr.Block.bh.Unmarshal(bsr.Block.headerData) // block header 反序列化
	if err != nil {
	...
	if bsr.Block.bh.ValuesBlockOffset != bsr.valuesBlockOffset {  // 第一次，bsr.valuesBlockOffset为 0， 但是第一个块的偏移量也一定为 0
	...
	if usePrevTimestamps {
		...
		fs.MustReadData(bsr.timestampsReader, bsr.Block.timestampsData)  // 读取 timestamps.bin
	...
	fs.MustReadData(bsr.valuesReader, bsr.Block.valuesData)  // 读取 values.bin

	// Update offsets.
	if !usePrevTimestamps {
		bsr.timestampsBlockOffset += uint64(bsr.Block.bh.TimestampsBlockSize)
	}
	bsr.valuesBlockOffset += uint64(bsr.Block.bh.ValuesBlockSize)  // 全局的 values.bin 中的偏移量
	bsr.indexBlockHeadersCount++

	return nil
}

func (bsr *blockStreamReader) readIndexBlock() error {  // 从 index.bin 读数据
	// Go to the next metaindex row.
	if len(bsr.mrs) == 0 {
		return io.EOF
	}
	bsr.mr = &bsr.mrs[0]  // 消费一个 metaindexRow
	bsr.mrs = bsr.mrs[1:]
	...
	bsr.indexData = tmpData  //??? 为什么不反序列化
