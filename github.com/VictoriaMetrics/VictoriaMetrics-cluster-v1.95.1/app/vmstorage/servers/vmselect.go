...
func NewVMSelectServer(addr string, s *storage.Storage) (*vmselectapi.Server, error) {
	...
	return vmselectapi.NewServer(addr, api, limits, *disableRPCCompression)  // vm-storage 上启动 vm-select 的 api 支持
}

// vmstorageAPI impelements vmselectapi.API
type vmstorageAPI struct {  // 这个对象，实现 vm-storage 上的 vm-select 的接口支持
	s *storage.Storage
}
...
func (api *vmstorageAPI) InitSearch(qt *querytracer.Tracer, sq *storage.SearchQuery, deadline uint64) (vmselectapi.BlockIterator, error) {
	...
	bi := getBlockIterator()
	bi.sr.Init(qt, api.s, tfss, tr, maxMetrics, deadline)  //  bi.sr storageSearch 对象
...
func (api *vmstorageAPI) SearchMetricNames(qt *querytracer.Tracer, sq *storage.SearchQuery, deadline uint64) ([]string, error) {
	tr := sq.GetTimeRange()
	maxMetrics := getMaxMetrics(sq)
	tfss, err := api.setupTfss(qt, sq, tr, maxMetrics, deadline)  // 内部使用了索引 1
	if err != nil {
		return nil, err
	}
	if len(tfss) == 0 {
...
func (api *vmstorageAPI) TSDBStatus(qt *querytracer.Tracer, sq *storage.SearchQuery, focusLabel string, topN int, deadline uint64) (*storage.TSDBStatus, error) {
	tr := sq.GetTimeRange()   // 查询 db 状态接口
	maxMetrics := getMaxMetrics(sq)
	tfss, err := api.setupTfss(qt, sq, tr, maxMetrics, deadline)  // tag filter
	if err != nil {
		return nil, err
	}
...
func (api *vmstorageAPI) setupTfss(qt *querytracer.Tracer, sq *storage.SearchQuery, tr storage.TimeRange, maxMetrics int, deadline uint64) ([]*storage.TagFilters, error) {
	tfss := make([]*storage.TagFilters, 0, len(sq.TagFilterss))  // 这个到底是 setup 啥啊 ???
	...
	for _, tagFilters := range sq.TagFilterss {
		...
		for i := range tagFilters {
			...
			if err := tfs.Add(tf.Key, tf.Value, tf.IsNegative, tf.IsRegexp); err != nil {  // 这里通过索引 1 来过滤标签
...
func (bi *blockIterator) NextBlock(mb *storage.MetricBlock) bool {  // 加载整个 block 到内存，并且未 decode
	if !bi.sr.NextMetricBlock() {
		return false
	}
	mb.MetricName = append(mb.MetricName[:0], bi.sr.MetricBlockRef.MetricName...)
	bi.sr.MetricBlockRef.BlockRef.MustReadBlock(&mb.Block)  // 把 timestamps.bin 和 values.bin 的数据加载到内存
	return true
}
