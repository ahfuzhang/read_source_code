...
// mergeBlockStreams merges bsrs into bsw and updates ph.  // 用于 data 的 merge， merge 的主流程
//
...
func mergeBlockStreams(ph *partHeader, bsw *blockStreamWriter, bsrs []*blockStreamReader, stopCh <-chan struct{}, s *Storage, retentionDeadline int64,
	...
	bsm := bsmPool.Get().(*blockStreamMerger)  // merge 对象
	bsm.Init(bsrs, retentionDeadline)  // 需要重点看：数据怎么被都出来的
	err := mergeBlockStreamsInternal(ph, bsw, bsm, stopCh, s, rowsMerged, rowsDeleted)  // 数据 merge 主流程
	bsm.reset()
	bsmPool.Put(bsm)
	bsw.MustClose()
	if err == nil {
...
func mergeBlockStreamsInternal(ph *partHeader, bsw *blockStreamWriter, bsm *blockStreamMerger, stopCh <-chan struct{}, s *Storage, rowsMerged, rowsDeleted *uint64) error {  // 数据 merge 主流程
	...
	for bsm.NextBlock() {  // 读下一个块。一个块就是一个 tsid
		select {
		case <-stopCh:
		...
		if b.bh.MaxTimestamp < retentionDeadline {  // 小于最小时间的数据不要
		...
		if pendingBlock.bh.TSID.MetricID != b.bh.TSID.MetricID {
			...
			bsw.WriteExternalBlock(pendingBlock, ph, rowsMerged)  // 当 tsid 不同的时候，把前一个块写入磁盘
			pendingBlock.CopyFrom(b)
			continue
		}
		...
		if err := unmarshalAndCalibrateScale(pendingBlock, b); err != nil {  // 反序列化 block 的数据
		...
		mergeBlocks(tmpBlock, pendingBlock, b, retentionDeadline, rowsDeleted)  // 把两个 block 俩俩合并
...
func mergeBlocks(ob, ib1, ib2 *Block, retentionDeadline int64, rowsDeleted *uint64) {  // 这个方法可以用于数据合并
	ib1.assertMergeable(ib2)  // 必须是同样的 tsid 才能合并
	ib1.assertUnmarshaled()   // 必须先调用 block.UnmarshalData
	...
	if ib1.bh.MaxTimestamp < ib2.bh.MinTimestamp {  // 从时间戳判断，数据一定不重复
		// Fast path - ib1 values have smaller timestamps than ib2 values.
		appendRows(ob, ib1)
		appendRows(ob, ib2)
		return
	}
	if ib2.bh.MaxTimestamp < ib1.bh.MinTimestamp {  // 从时间戳判断，数据一定不重复
		// Fast path - ib2 values have smaller timestamps than ib1 values.
		appendRows(ob, ib2)
		appendRows(ob, ib1)
		return
	}
	if ib1.nextIdx >= len(ib1.timestamps) {  // ib1 已经遍历完成了
		appendRows(ob, ib2)
		return
	}
	if ib2.nextIdx >= len(ib2.timestamps) {  // ib2 已经遍历完成了
		appendRows(ob, ib1)
		return
	}
	for {  // ??? 为什么没有对时间戳完全相等的数据去重
		i := ib1.nextIdx
...
func unmarshalAndCalibrateScale(b1, b2 *Block) error {
	if err := b1.UnmarshalData(); err != nil {  // 把 block 的数据变成 数组
		return err
	}
