...
func (api *API) serveSend(w http.ResponseWriter, r *http.Request) {
	...
	fmt.Println("alert-manager post len: ", r.ContentLength)
	...
	fmt.Println("alerts:", len(promMessage.Alerts))
	if len(promMessage.Alerts) > 30 {
		promMessage.Alerts = promMessage.Alerts[:30]
	}
